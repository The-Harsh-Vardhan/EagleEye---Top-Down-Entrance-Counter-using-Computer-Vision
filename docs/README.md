# EagleEye Documentation

Complete documentation for the EagleEye People Counting System.

## 📚 Documentation Files

### For Users

- **[INSTALLATION.md](INSTALLATION.md)** - Complete installation guide
  - Platform-specific instructions (Windows, Linux, macOS)
  - GPU/CUDA setup
  - Hardware configuration
  - Troubleshooting common issues

- **[EXAMPLES.md](EXAMPLES.md)** - Real-world usage examples
  - Basic usage scenarios
  - Advanced configurations
  - Integration patterns (REST API, MQTT)
  - Multi-camera setups
  - Analysis and reporting

### For Developers

- **[API_REFERENCE.md](API_REFERENCE.md)** - Complete API documentation
  - Module documentation
  - Class and function references
  - Parameter descriptions
  - Code examples
  - Type definitions

## 🚀 Quick Links

### Getting Started
1. Read [Installation Guide](INSTALLATION.md)
2. Try basic examples from [Examples](EXAMPLES.md)
3. Review [API Reference](API_REFERENCE.md) for advanced usage

### Common Tasks

| Task | Documentation |
|------|---------------|
| Install EagleEye | [INSTALLATION.md](INSTALLATION.md) |
| Basic usage | [../README.md](../README.md#usage) |
| Advanced examples | [EXAMPLES.md](EXAMPLES.md) |
| Understand the code | [API_REFERENCE.md](API_REFERENCE.md) |
| Contribute | [../CONTRIBUTING.md](../CONTRIBUTING.md) |

## 📖 Other Resources

- **[Main README](../README.md)** - Project overview and quick start
- **[CONTRIBUTING](../CONTRIBUTING.md)** - Contribution guidelines
- **[CHANGELOG](../CHANGELOG.md)** - Version history
- **[Academic Documentation](../documentation.md)** - Research context

## 🆘 Need Help?

- Check [INSTALLATION.md](INSTALLATION.md#troubleshooting) for common issues
- Search [GitHub Issues](https://github.com/The-Harsh-Vardhan/EagleEye/issues)
- Start a [Discussion](https://github.com/The-Harsh-Vardhan/EagleEye/discussions)

---

[⬆ Back to Main README](../README.md)
